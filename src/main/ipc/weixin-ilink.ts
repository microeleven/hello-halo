...
export function registerWeixinIlinkHandlers(): void {
   ...
   */
  ipcMain.handle('weixin-ilink:request-qrcode', async () => {
    try {
   ...
   *     bot_token?: string,
   *     ilink_bot_id?: string,   // accountId — required on 'confirmed'
   *     baseurl?: string,
   ...
   */
  ipcMain.handle('weixin-ilink:poll-auth-status', async (_event, qrcode: string) => {
    try {
      if (!qrcode) {
   ...
   */
  ipcMain.handle(
    'weixin-ilink:save-token',
    async (_event, instanceId: string, botToken: string, baseUrl?: string, accountId?: string) => {
      try {
   ...
   */
  ipcMain.handle('weixin-ilink:disconnect', async (_event, instanceId: string) => {
    try {
